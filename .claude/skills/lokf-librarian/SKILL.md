...
2. **The bundle-root `index.md` carries the semantic header.** It declares the keys that lift the whole bundle into RDF (values shown are illustrative - the real ones are minted at scaffolding time):
   ...
   **Choosing `base_iri` - identifiers, not hyperlinks, but pick a namespace you control.** A concept's `id` is a globally unique *name*, like a namespaced UUID that happens to look like a URL - nothing in LOKF requires that pasting it into a browser return a page, so a 404 on a concept `id` is *valid*. It is still a design smell worth flagging: Linked Data best practice ("Cool URIs") is that these identifiers *should* eventually double as working links. The test for a good `base_iri` is **authority + future resolvability**:
   - **Never mint inside a URL space the project doesn't control.** Paths under a code host's routing scheme (e.g. `https://github.com/<org>/<repo>/knowledge/...`) can *never* be made to resolve - the host owns that path space - and they misattribute naming authority to the host. The same applies to any third-party domain.
   - **Prefer, in order:** (a) a namespace the project already publishes under - if the repo's schemas/ontologies use a persistent-identifier namespace (w3id.org, purl.org, a project-owned domain), put the bundle under it (e.g. `https://w3id.org/<org>/<project>/knowledge/`); (b) a new persistent-identifier registration (w3id.org rules are a small PR to `perma-id/w3id.org`, redirectable later to rendered pages, e.g. GitHub Pages); (c) a project-owned domain. Repo cues: schema `id`/namespace declarations, a docs `site_url`, a Pages deployment.
   - **Migrate early if the base is wrong.** Changing `base_iri` rewrites every concept `id` and breaks external links to the old identifiers - cheap while the bundle is young and unpublished, increasingly expensive after. When migrating: replace the namespace in `base_iri`, the publisher `id`, every concept `id`, and all typed-relation targets; leave `resource`/`distribution` URLs alone (those are real links, not minted identifiers); log the migration and its rationale in `log.md`; re-run `just lokf-validate` and confirm the converted graph output contains only the new namespace.
   - **When a human asks "these IDs don't resolve - is that a problem?"** the answer is: valid by design, but check the two-part test above; if the namespace is uncontrolled it can never resolve and should be migrated, and if it is controlled but unregistered, note the pending redirect registration rather than treating the 404 as a defect.
...
On every concept you create or materially change, set/refresh its `timestamp` (`schema:dateModified`) - but never bump timestamps on untouched concepts, or the diff fills with churn. Update the nearest `index.md` (bullet + `description`) and prepend a dated entry to `log.md` (newest first, ISO `YYYY-MM-DD`). `log.md` records **knowledge changes only** - concepts added/changed/removed, or the source map updated. If a run changes nothing in the bundle, write no log entry; never log administrative events ("librarian ran, no changes detected") - they do not represent a knowledge change.
